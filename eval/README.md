...
- ✅ **In-domain study (first cut):** [`in-domain-study/`](in-domain-study/FINDINGS.md) — the `research-and-pick-a-stock` playbook **vs** baseline **vs** the Street, on 4 large-caps (Intel / Tesla / Boeing / Disney). Finding: the playbook adds **real analytical discipline** (forces the edge / forecastability / pricing / asymmetry gates a baseline glosses) **but inherits its source book's worldview** — it's a *veto-machine* (always "right-but-priced → no trade", never Buy) and produced a **false negative on Disney** (vetoed a Buy that the Street, JP Morgan +50%, and the baseline all saw). The eval caught it — teeth confirmed. *Deep result: a distilled skill carries the book's bias, not just its method.*
- 🔭 **Next:** worldview **composition** (compose skills across multiple books to counter single-book bias), and an **outcome-scored** version (do the calls actually make/save money) — the harder frontier.
